export function calculateLandsFromSymbols(
  numLands,
  blueSymbols,
  redSymbols,
  greenSymbols,
  ...
  if (totalSymbols !== 0) {
    ...
    numLandsJson.greenLands = Math.round(
      (greenSymbols * numLands) / totalSymbols
    );
    numLandsJson.blackLands = Math.round(
      (blackSymbols * numLands) / totalSymbols
    );
    numLandsJson.whiteLands = Math.round(
      (whiteSymbols * numLands) / totalSymbols
    );
    ...
    for (const landType in numLandsJson) {
      ...
      if (numLandsJson[landType] > mostLandCount) {
        ...
        numLandsJson[landType] !== 0 &&
        numLandsJson[landType] < leastLandCount
      ) {
